...
function confirmBoyakiAction(message,title='確認',confirmLabel='取り下げる'){
  const overlay=document.querySelector('#boyaki-alert');
  if(!overlay)return Promise.resolve(confirm(message));
  const titleNode=overlay.querySelector('[data-alert-title]');
  const messageNode=overlay.querySelector('[data-alert-message]');
  const ok=overlay.querySelector('[data-alert-close]');
  if(!titleNode||!messageNode||!ok)return Promise.resolve(confirm(message));
  const actions=ok.parentElement;
  const cancel=document.createElement('button');
  cancel.type='button';cancel.textContent='キャンセル';
  ok.textContent=confirmLabel;
  actions.insertBefore(cancel,ok);
  titleNode.textContent=title;messageNode.textContent=message;
  overlay.hidden=false;requestAnimationFrame(()=>overlay.classList.add('show'));
  return new Promise(resolve=>{
    let done=false;
    const finish=value=>{
      if(done)return;done=true;
      cancel.remove();ok.textContent='OK';
      overlay.classList.remove('show');setTimeout(()=>overlay.hidden=true,160);
      resolve(value);
    };
