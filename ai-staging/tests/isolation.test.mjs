...
const mypageHtml=await read('mypage.html'),mypageJs=await read('mypage.js');
assert.match(mypageHtml,/id="account-key-reissue-form"/);
assert.match(mypageHtml,/id="account-key-new-password"[^>]*minlength="10"/);
assert.match(mypageHtml,/古いログインキーを無効化する/);
assert.match(mypageJs,/nip49\.encrypt\(account\.sk,nextPassword\)/);
assert.match(mypageJs,/getPublicKey\(verifiedSk\)!==account\.pk/);
assert.match(mypageJs,/target\.setItem\('boyaki-account-login-key',loginKey\)/);
