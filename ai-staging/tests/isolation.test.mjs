...
for(const file of files.filter(f=>f.endsWith('.html'))){const text=await read(file);if(text.includes('<html')){assert(/<title>[^<]+<\/title>/.test(text),file+' missing title');assert(/<body[\s>]/.test(text),file+' missing body')}}
const homeHtml=await read('index.html');
for(const required of ['<form id="raw-form">','id="raw"','id="submit-btn"','id="status"','id="problem-view"','id="feed-view"','id="feed"','data-onboarding-open','href="./discover/"','href="./products.html"','href="./mypage.html"','src="./chat-first-v53.js','src="./onboarding.js','src="./canonical-api.js','src="./app.js'])assert(homeHtml.includes(required),'home surface missing '+required);
assert(!homeHtml.includes('train-ticketing')&&!homeHtml.includes('work-admin'),'obsolete homepage campaign overlay must stay removed');
// legacy-write-freeze.js swallows #join-form/#activity-form/#room-form submits unless the page explicitly declares its canonical write path.
for(const file of files.filter(f=>f.endsWith('.html'))){const html=await read(file);if(!html.includes('legacy-write-freeze.js')||!/id="(?:join-form|activity-form|room-form)"/.test(html))continue;
  const own=[];for(const m of html.matchAll(/<script[^>]*src="(\.\/[^"?#]+)/g))if(!m[1].endsWith('legacy-write-freeze.js'))own.push(await read(new URL(m[1],new URL(file,root)).href.slice(root.href.length)));
  assert(own.some(src=>/window\.BOYAKI_MAKERSPACE_CANONICAL_WRITE_ACTIVE\s*=\s*true/.test(src)),file+' canonical form is swallowed by legacy-write-freeze.js')}
const freezeContext={window:{},document:{listeners:[],addEventListener(type,fn,capture){this.listeners.push({type,fn,capture})},querySelector:()=>null},HTMLFormElement:class{}};
vm.runInNewContext(await read('legacy-write-freeze.js'),freezeContext);
const roomForm=Object.assign(new freezeContext.HTMLFormElement(),{id:'room-form',matches:s=>s.includes('#room-form'),querySelector:()=>null});
const submitThrough=()=>{let blocked=false;for(const l of freezeContext.document.listeners)l.fn({target:roomForm,preventDefault(){blocked=true},stopImmediatePropagation(){}});return!blocked};
assert.equal(submitThrough(),false,'legacy room form must stay frozen by default');
freezeContext.window.BOYAKI_MAKERSPACE_CANONICAL_WRITE_ACTIVE=true;assert.equal(submitThrough(),true,'declared canonical room form must pass the freeze');
// Every AI-STAGING API a page's scripts can call must be allowed by that page's CSP connect-src; otherwise the browser silently refuses the request.
const apiClientSource=await read('canonical-api.js'),apiBases={main:'ai-staging-boyaki-api',thread:'ai-staging-boyaki-thread-api',commerce:'ai-staging-commerce-api',inbox:'ai-staging-inbox-api'},clientMethodApi=new Map();
for(const m of apiClientSource.matchAll(/^\s*([A-Za-z]+):(?:\([^)]*\)|[A-Za-z_$][\w$]*)=>(main|thread|commerce|inbox)\(/gm))clientMethodApi.set(m[1],apiBases[m[2]]);
assert(clientMethodApi.get('listPostProducts')==='ai-staging-commerce-api'&&clientMethodApi.size>=40,'client method -> API map incomplete');
for(const file of files.filter(f=>f.endsWith('.html'))){const html=await read(file);if(!html.includes('<html')||/window\.BOYAKI_CANONICAL\s*=\s*\{/.test(html))continue;// pages that inline a mock client make no API calls
  const csp=(/connect-src([^;"]*)/.exec(html)||[,''])[1].split(/\s+/),seen=new Set(),queue=[...html.matchAll(/<script[^>]*src="(\.\.?\/[^"?#]+)/g)].map(m=>new URL(m[1],new URL(file,root)).href);
  if(/id="feed"/.test(html))queue.push(new URL('canonical-cutover.js',root).href);// canonical-api.js imports the cutover only on pages with #feed
  const needed=new Set();
  while(queue.length){const href=queue.shift();if(seen.has(href)||!href.endsWith('.js'))continue;seen.add(href);const src=await readFile(new URL(href),'utf8');
    for(const m of src.matchAll(/(?:\bfrom\s*|^\s*import\s*)['"](\.\.?\/[^'"?#]+)/gm))queue.push(new URL(m[1],href).href);// static imports only
    for(const m of src.matchAll(/client\.([A-Za-z]+)\(/g))if(clientMethodApi.has(m[1]))needed.add(clientMethodApi.get(m[1]))}
  for(const api of needed)assert(csp.includes(base+api+'/'),`${file} CSP connect-src blocks ${api} used by its scripts`)}
...
assert(/client\.createSolutionRoomMessage\(room,[A-Za-z_$][\w$]*\)/.test(solutionRoomSource),'Solution Room isolated write missing');
...
assert(/inviteeContext='source_owner'/.test(invitationThreadSource)&&invitationThreadSource.includes('invitee_context:inviteeContext')&&invitationThreadSource.includes("invitation.invitee_context==='source_owner'"),'source-owner invitation context missing');
...
assert(cutoverInviteSource.includes('client.acceptSolutionRoomInvitation(access.my_invitation.id,textarea.value.trim(),true)')&&(await read('canonical-api.js')).includes('body:{problem_statement,confirm_shared_problem}'),'shared Problem consent payload missing');
...
assert(cutoverInviteSource.includes('この困りごとで、どちらの立場から関わるかを選びます')&&cutoverInviteSource.includes('困っている・試したい・条件を伝える側')&&cutoverInviteSource.includes('質問し、解決を作って届ける側'),'Voice/Maker contextual role explanation missing');
