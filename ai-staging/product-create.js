...
function renderCase(item){
  ...
  const e=document.createElement('p');e.className='eyebrow';e.textContent='元の解決メモ';
  const h=document.createElement('h2');h.textContent=item.title||'解決メモ';
  ...
  if(source){const q=document.createElement('p');q.className='hint';q.textContent=`${post?.source_withdrawn?'残った困りごと（元BOYAKI本文は取り下げ済み）':'元のBOYAKI'}: ${source}`;box.append(q)}
}
async function load(){
  const id=client.identity();
  if(id?.kind!=='account'){location.href='./mypage.html#maker';return}
  if(!uuid.test(caseId)){ $('#product-case').innerHTML='<p class="hint">解決メモのIDが不正です。</p>';$('#product-form').hidden=true;return}
  try{
    ...
    if(!caseRow){$('#product-case').innerHTML='<p class="hint">このAccount IDの解決メモではありません。</p>';$('#product-form').hidden=true;return}
  ...
  }catch(err){console.error(err);$('#product-case').innerHTML='<p class="hint">解決メモを読み込めませんでした。</p>';$('#product-form').hidden=true}
}
$('#product-form').addEventListener('submit',async e=>{
  ...
  button.disabled=true;button.textContent='作成中…';$('#product-status').textContent='Productを作成しています…';
  try{
    const result=await client.createProduct(caseRow.id,title,description,price,delivery);
    $('#product-status').textContent='Productを作成しました。次の画面で元の困りごとに掲載するか選べます。';
    ...
    $('#product-status').textContent=code==='case_already_productized'?'この解決メモはすでにProductになっています。':'Productを作成できませんでした。入力内容と通信状態を確認してください。';
    button.disabled=false;button.textContent='Productを作成';
  }
});
await load();