...
let cleanupAccount:Id|null=null,cleanupPost='',cleanupMessage='';
async function run(){
...
try{return new Response(JSON.stringify(await run()),{headers:{'content-type':'application/json',...cors}})}catch(e){results.push({name:'execution',status:'FAIL',detail:String((e as any)?.message||e)});for(const [base,path] of [[THREAD,cleanupMessage?`/thread/${cleanupMessage}`:''],[API,cleanupPost?`/posts/${cleanupPost}`:'']]){if(path&&cleanupAccount)try{await req(base,path,{method:'DELETE',id:cleanupAccount})}catch{}}return new Response(JSON.stringify({ok:false,error:String((e as any)?.message||e),results,responses}),{status:500,headers:{'content-type':'application/json',...cors}})}});