...
async function run(){
 ...
 const roomAgain=(await req(THREAD,`/posts/${created.id}/solution-room`,{method:'POST',id:b,expected:[200]})).p;eq(roomAgain.room.id,roomId,'solution room idempotent');pass('solution-room-idempotent');
 const rooms=(await req(THREAD,'/solution-rooms')).p.rooms||[];yes(rooms.some((x:any)=>x.id===roomId&&x.post?.id===created.id),'solution room list binding');pass('solution-room-list');
 ...
 const accessAfterRoom=(await req(THREAD,`/posts/${created.id}/thread/access`,{id:b})).p;eq(accessAfterRoom.solution_room_id,roomId,'thread access room link');pass('thread-solution-room-link');
 ...
 const casesAfter=(await req(THREAD,'/me/solution-cases',{id:a})).p.cases||[];yes(!casesAfter.some((x:any)=>x.id===createdCase.id),'deleted solution case still visible');pass('solution-case-deleted-hidden');
