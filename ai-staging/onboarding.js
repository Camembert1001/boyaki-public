(()=>{
  ...
  const steps=[
    ...
    ['2','公開する','自分で公開を選んだBOYAKIだけがThreadに出ます。同じ痛みを持つVoiceや、解けるMakerが集まります。'],
    ...
    ['5','Productになる','MakerがCaseをProductに変え、Maker Spaceに残します。完成したProductは元のProblemへ掲載できます。'],
  ...
  voice.append(add('strong','Voice'),add('span','困っている人、試す人、条件を伝える人。Problemの需要を具体化します。'));
  ...
  casual.append(add('strong','まだ共同解決に進んでいない'),add('span','あなたのBOYAKIです。削除できます。MakerがRoomを準備していても、Shared Problemになる前なら共同資産化しません。'));
  const shared=add('div','', 'onboarding-boundary-card');
  shared.append(add('strong','共同解決へ進むことに同意した後'),add('span','元のBOYAKI本文は後から取り下げられます。一方、あなたが確認した一般化Problem、他の人の発言、Solution、Productは残る場合があります。'));
