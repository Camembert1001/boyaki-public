...
function render(p){
  ...
  st.textContent=source?.source_withdrawn?'残った困りごと（元BOYAKI本文は取り下げ済み）':'元のBOYAKI';
  sourceBox.append(st);
  const sp=document.createElement('p');sp.textContent=sourceText||'元のBOYAKIは取り下げ済み';sourceBox.append(sp);box.append(sourceBox);
  if(p.solution_case){const caseBox=document.createElement('div');caseBox.className='candidate-block';caseBox.innerHTML='<strong>解決メモ</strong>';const t=document.createElement('p');t.textContent=p.solution_case.title;const c=document.createElement('p');c.className='hint';c.textContent=p.solution_case.contribution;caseBox.append(t,c);box.append(caseBox)}
  const actions=document.createElement('div');actions.className='actions';
  if(p.room?.id){const room=document.createElement('a');room.className='button-link';room.href=`./solution-room.html?room=${encodeURIComponent(p.room.id)}&mode=view`;room.textContent='話し合いの記録を見る';actions.append(room)}
  box.append(actions);
}
...
async function load(){
  if(!uuid.test(id)){ $('#product-detail').innerHTML='<p class="hint">Product IDが不正です。</p>';return}
  try{
    const result=await client.getProduct(id);product=result.product;render(product);document.title=`${product.title} — BOYAKI`;
    ...
    if(identity?.kind==='account'&&identity.pk===product.maker_account_pubkey){
      ...
      if(product.thread_publication?.status==='active'){
        button.hidden=true;link.hidden=false;link.href=`./?problem=${encodeURIComponent(product.thread_publication.post_id)}`;state.textContent='元の困りごとに掲載済みです。';
      }else{
        button.hidden=false;link.hidden=true;state.textContent='Productは作成済みです。まだ元の困りごとには掲載していません。';
      }
    }
    if(identity?.kind==='account'){
      ...
      if(!has){
        ...
        if(identity.pk===product.maker_account_pubkey){$('#checkout-card').innerHTML='<p class="hint">これはあなたが作ったProductです。購入せず受け取り内容を確認できます。</p>';await showAccess()}
  ...
  }catch(err){console.error(err);$('#product-detail').innerHTML='<p class="hint">Productを読み込めませんでした。</p>'}
}
$('#publish-back')?.addEventListener('click',async()=>{
  if(!product)return;
  const button=$('#publish-back'),state=$('#publish-back-status'),link=$('#published-thread-link');
  button.disabled=true;button.textContent='掲載中…';state.textContent='元の困りごとへProductを掲載しています…';
  try{
    ...
    state.textContent=result.idempotent?'すでに元の困りごとへ掲載済みでした。':'元の困りごとにProductを掲載しました。困っていたVoiceがここから見つけられます。';
  }catch(err){
    console.error('publish back failed',err);const code=String(err?.message||err);
    state.textContent=code==='source_thread_not_available'?'元の困りごとが残っていないため掲載できません。':'元の困りごとへ掲載できませんでした。';
    button.disabled=false;button.textContent='元の困りごとに掲載する';
  }
});
$('#buy-product')?.addEventListener('click',async()=>{
  const button=$('#buy-product');button.disabled=true;button.textContent='購入処理中…';$('#checkout-status').textContent='テスト購入を処理しています…';
  try{
    ...
    await showAccess();
  }catch(err){console.error(err);const code=String(err?.message||err);$('#checkout-status').textContent=code==='maker_cannot_buy_own_product'?'自分のProductは購入できません。':'購入処理に失敗しました。';button.disabled=false;button.textContent=`${yen(product?.price_yen)}でテスト購入する`;}
});
await load();