...
async function report(eventId){
  const id=identity();
  const ev=finalizeEvent({kind:1984,created_at:Math.floor(Date.now()/1000),content:'BOYAKI Solution Room message report',tags:[['e',eventId],['app','boyaki-web'],['schema','solution-room-report-v1']]},id.sk);
...
function apply(root=document){
  const chats=[];
  if(root.matches?.('.solution-room-chat'))chats.push(root);
  root.querySelectorAll?.('.solution-room-chat').forEach(x=>chats.push(x));
  for(const chat of chats){
    for(const item of chat.querySelectorAll('.activity-item')){
      const old=[...item.querySelectorAll('button')].find(b=>b.textContent.trim()==='取り下げ');
      if(!old||old.dataset.reportPolicy==='1')continue;
      const button=old.cloneNode(true);
      button.textContent='問題を報告';button.dataset.reportPolicy='1';
      old.replaceWith(button);
      button.addEventListener('click',async()=>{
        const eventId=item.dataset.eventId||'';
        if(!eventId){button.textContent='報告できませんでした';button.disabled=true;return}
        button.disabled=true;button.textContent='報告中…';
        try{await report(eventId);button.textContent='報告しました'}catch(err){console.error(err);button.disabled=false;button.textContent='問題を報告'}
      });
    }
  }
}
