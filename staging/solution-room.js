...
function renderMessage(ev,{pending=false}={}){if(seen.has(ev.id))return;const data=parse(ev),message=String(data.message||data.note||'').trim();if(!message)return;seen.add(ev.id);const log=$('#room-log');log.querySelector('.room-empty')?.remove();const mine=identity?.pk===ev.pubkey;const item=document.createElement('div');item.className=`room-message${mine?' mine':''}`;item.dataset.eventId=ev.id;item.innerHTML=`<div class="room-meta">${escapeHtml(data.displayName||`${ev.pubkey.slice(0,8)}…${ev.pubkey.slice(-6)}`)} · ${escapeHtml(fmt(ev.created_at))}${mine?' · you':''}${pending?' · 送信中':''}</div><p>${escapeHtml(message)}</p>`;log.append(item);item.scrollIntoView({block:'nearest'})}
async function load(){if(!validRoom)return;const log=$('#room-log');log.innerHTML='<p class="hint">復元しています…</p>';seen.clear();try{const since=Math.floor(Date.now()/1000)-60*60*24*365;const [messages,deletions]=await Promise.all([pool.querySync(RELAYS,{kinds:[1],'#t':['boyaki-solution-room-message'],'#room_id':[room],since,limit:500}),pool.querySync(RELAYS,{kinds:[5],since,limit:1000})]);const deleted=new Map();for(const d of deletions){let set=deleted.get(d.pubkey);if(!set){set=new Set();deleted.set(d.pubkey,set)}for(const t of d.tags||[])if(t[0]==='e'&&t[1])set.add(t[1])}const visible=messages.filter(ev=>!deleted.get(ev.pubkey)?.has(ev.id)).sort((a,b)=>a.created_at-b.created_at||String(a.id).localeCompare(String(b.id)));log.innerHTML='';if(!visible.length)log.innerHTML='<p class="hint room-empty">まだSolution Logはありません。最初のメッセージを送れます。</p>';for(const ev of visible)renderMessage(ev)}catch(err){log.innerHTML=`<p class="hint">Solution Logを復元できませんでした。${escapeHtml(err?.message||'')}</p>`}}
function startRealtime(){if(!validRoom)return;try{sub?.close?.()}catch{}try{sub=pool.subscribeMany(RELAYS,[{kinds:[1],'#t':['boyaki-solution-room-message'],'#room_id':[room],since:Math.floor(Date.now()/1000)}],{onevent(ev){renderMessage(ev)},oneose(){}})}catch(err){console.warn('realtime subscribe failed',err)}}
...
$('#solution-case-create')?.addEventListener('click',()=>{location.href=`./solution-case-create.html?room=${encodeURIComponent(room)}`;});
