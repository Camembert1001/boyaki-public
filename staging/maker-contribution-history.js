...
if(box){
  ...
  if(cases.length){
    ...
    for(const c of cases){
      ...
      if(c.room_id){const actions=document.createElement('div');actions.className='actions';const link=document.createElement('a');link.className='button-link';link.href=`./solution-room.html?room=${encodeURIComponent(c.room_id)}`;link.textContent='Solution Logを見る';actions.append(link);item.append(actions)}
      box.append(item);
    }
  }
}