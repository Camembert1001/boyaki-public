...
$('#device-id').textContent=`${identity.type==='account'?'Account':'Legacy browser'} · ${short(identity.pk)}`;
...
function parseContent(ev){try{return JSON.parse(ev.content||'{}')}catch{return {note:ev.content||''}}}
...
function roleLabel(role){return ({voice:'Voice',maker:'Maker',tester:'Voice / Tester',contributor:'Contributor','problem-owner':'Voice / Problem owner'})[role]||role||'Contributor'}
...
function activityLabel(type){return ({'test-result':'Test / Validation','build-progress':'Build / Implementation',research:'Problem / Research',review:'Voice feedback / Review',blocked:'Blocked / Limitation'})[type]||type||'Solution update'}
function bilingual(ja,en){return `${escapeHtml(ja||'')}<br><span class="hint">${escapeHtml(en||'')}</span>`}
...
function renderTimeline(candidate){
  ...
  wrap.innerHTML='<strong>Case history / Caseの経緯</strong>';
  ...
  for(const step of candidate.timeline||[]){
    ...
    item.innerHTML=`<div class="activity-meta"><span class="chip">${escapeHtml(step.status||'recorded')}</span> ${label}</div>`;list.append(item);
  }
  if(!(candidate.timeline||[]).length)list.innerHTML='<p class="hint">まだCase historyはありません。</p>';
...
function renderRoleGuide(candidate){
  ...
  wrap.innerHTML='<strong>Participants / このCaseに関わる人</strong>';
  const grid=document.createElement('div');grid.className='principles-grid';
  const roles=candidate.participation_roles||{};
  ...
  if(!grid.children.length)grid.innerHTML='<span class="hint">Voiceは課題・検証・フィードバックを、Makerは解決の設計・実装を担います。</span>';
  wrap.append(grid);return wrap;
}

async function loadRegistry(){
  const r=await fetch('./solution-candidates.json',{cache:'no-store'});
  if(!r.ok)throw new Error('solution room registry unavailable');
...
async function loadEvents(){
  ...
  for(const deletion of deletions){
    let ids=deletedByOwner.get(deletion.pubkey);if(!ids){ids=new Set();deletedByOwner.set(deletion.pubkey,ids)}
    for(const t of deletion.tags||[])if(t[0]==='e'&&t[1])ids.add(t[1]);
  }
  const dedupe=new Map();
  for(const ev of work){const ownerDeletes=deletedByOwner.get(ev.pubkey);if(!ownerDeletes?.has(ev.id))dedupe.set(ev.id,ev)}
  const suppressed=new Set();
  try{
    const sr=await fetch('./public-surface-suppressions.json',{cache:'no-store'});
    if(sr.ok){const data=await sr.json();for(const row of data.events||[]){const id=row&&row.event_id;if(typeof id==='string'&&id.length===64&&row.scope==='boyaki_public_surface_only')suppressed.add(id)}}
  }catch{}
...
function renderParticipants(card,candidate){
  ...
  if(!people.length){wrap.innerHTML='<p class="hint">まだ参加者はいません。VoiceまたはMakerとしてこのSolution Roomに参加できます。</p>';return}
  wrap.innerHTML='<h3>Participants / 参加者</h3><div class="participant-list"></div>';
...
function renderActivity(card,candidate){
  ...
  wrap.innerHTML='<h3>Solution Log</h3>';
  if(!xs.length){wrap.insertAdjacentHTML('beforeend','<p class="hint">まだSolution Logはありません。調査、実装、検証、Voice feedback、詰まりを時系列で残せます。</p>');return}
  const list=document.createElement('div');list.className='activity-list';
  for(const ev of xs){
...
function renderCandidate(candidate){
  ...
  $('.candidate-status',card).textContent=`Solution Room · ${candidate.status_label_ja||candidate.status_label_en||'active'}`;
  $('.candidate-title',card).textContent=candidate.title;
  $('.candidate-id',card).textContent=`Case ${candidate.id}`;
  ...
  const detailBlocks=[
    ...
    caseBlock('Target / 目指す状態',candidate.target_ja||'',candidate.target_en||''),
    caseBlock('Work so far / ここまで試したこと',candidate.tried_ja||'',candidate.tried_en||''),
    renderTimeline(candidate),
    caseBlock('Outcome / 現時点の成果',candidate.outcome_ja||'',candidate.outcome_en||'','verified'),
    renderRoleGuide(candidate)
  ];
  let anchor=summary;for(const block of detailBlocks){anchor.insertAdjacentElement('afterend',block);anchor=block}
...
function render(){
  ...
  if(!registry.length){list.innerHTML='<div class="card">進行中のSolution Roomはまだありません。</div>';return}
  for(const c of registry)list.append(renderCandidate(c));
}
function revealPanel(panel){
  panel.hidden=false;
  requestAnimationFrame(()=>requestAnimationFrame(()=>{panel.scrollIntoView({behavior:'smooth',block:'center'});const focusable=panel.querySelector('textarea,input:not([type="hidden"]),select');if(focusable)focusable.focus({preventScroll:true})}));
}
function openJoin(candidate){selectedCandidate=candidate;$('#join-title').textContent=`${candidate.title} のSolution Roomに参加`;$('#activity-panel').hidden=true;$('#join-status').textContent='';revealPanel($('#join-panel'))}
function openActivity(candidate){selectedCandidate=candidate;$('#activity-title').textContent=`${candidate.title} — Solution Logを追加`;$('#join-panel').hidden=true;$('#activity-status').textContent='';revealPanel($('#activity-panel'))}
...
$('#activity-form').addEventListener('submit',async e=>{
  e.preventDefault();if(!selectedCandidate)return;
  const button=e.submitter,activityType=$('#activity-type').value,displayName=$('#activity-name').value.trim(),note=$('#activity-note').value.trim();
  if(!note){$('#activity-status').textContent='Solution Logを書いてから追加してください。';return}
