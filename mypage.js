...
async function main(){
  ...
  if(profile.displayName){$('#profile-name').textContent=profile.displayName;$('#profile-about').textContent=profile.about||'';$('#profile-state').textContent='BOYAKIアカウントにログイン中です。';$('#profile-actions').innerHTML='<a class="button-link" href="./profile-edit.html">プロフィールを編集</a><button id="logout-button" type="button">ログアウト</button>';setTimeout(()=>{const b=$('#logout-button');if(b)b.addEventListener('click',()=>{localStorage.removeItem('boyaki-account-sk');sessionStorage.removeItem('boyaki-account-sk');location.reload()})},0)}else{$('#profile-name').textContent='プロフィール未登録';$('#profile-state').textContent='アカウント鍵はありますが、プロフィールを取得できませんでした。';$('#profile-actions').innerHTML='<a class="button-link" href="./register.html">プロフィールを作る</a>'}
